...
from .services import export_service, import_service, table_service
...
@shared_task(name="forms_builder.chay_tac_vu_tinh_lai")
def chay_tac_vu_tinh_lai(job_id):
    """Tính lại cột tính sẵn / cột tách của một bảng lớn — ADR-016."""
    job = table_service.run_resync_job(job_id)
    return job.status if job else "bo_qua"
