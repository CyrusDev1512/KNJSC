...
def page_size(request, param="moi_trang"):
    """Số dòng mỗi trang, chỉ nhận các giá trị đã cho phép."""
    mac_dinh = getattr(settings, "PAGE_SIZE_DEFAULT", 25)
    try:
...
def paginate(request, queryset, param="trang", size_param="moi_trang"):
    ...
    """
    paginator = Paginator(queryset, page_size(request, size_param))
