...
def test_trang_chu_kn_crm_chon_thang_va_quay_lai(live_server, trang, dang_nhap, du_lieu, nguoi_dung):
    """AC-11.29 — Trang thư mục KN CRM (mục Bảng tính trên sidebar): cây Bộ phận ▸ Quý ▸ Tháng; bấm Tháng 8/2026 rồi Mở là lưới lọc đúng tháng có nhãn tháng; nút ← về đúng nhánh đang mở, có sidebar"""
    dang_nhap(trang, nguoi_dung["staff_vd"])
    trang.goto(live_server.url + "/")
    assert trang.locator("#thanh-ben").count() == 1 and trang.locator("a.bt-ve").count() == 0
    chup(trang, "kn-crm-tong-quan")
    trang.locator("#thanh-ben a.crm-nhom-lk", has_text="Bảng tính").click()
    trang.wait_for_url("**/thu-muc/")
    assert trang.locator("h2.crm-tieu-de").inner_text().startswith("Vận đơn")
    thang8 = trang.locator("a.crm-thang-lk", has_text="Tháng 8/2026")
    assert thang8.inner_text().strip().endswith("4")           # bốn dòng tháng 8
    thang8.click()
    trang.wait_for_url("**/thu-muc/?bp=van-don&thang=2026-08")
