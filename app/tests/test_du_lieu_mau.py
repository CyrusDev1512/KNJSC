...
def test_dung_du_co_cau_to_chuc():
    """FR-2.1, FR-2.2 — Ba bộ phận nghiệp vụ, ba team (Sale 1, Sale 2, MKT 1 — ADR-043), mỗi team một trưởng nhóm
    ...
    mẫu. Khẳng định đủ bốn để bài kiểm đỏ nếu ai đó lỡ xoá nó.
    """
    _chay()
...
def test_du_lieu_noi_bo_va_bang_xep_hang_mau():
    ...
    for gn in Recognition.objects.select_related("giver__profile", "receiver__profile"):
        assert rank_level(gn.giver.profile.rank) > rank_level(gn.receiver.profile.rank), gn   # Q75
