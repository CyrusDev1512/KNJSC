"""Ghi nhận và sao — FR-12.1, FR-12.2, FR-12.5, ADR-017.

Tầng dịch vụ, không biết gì về HTTP (điều cấm 2). Sổ sao chỉ ghi thêm.
Ghi nhận đi **từ trên xuống** (Q75, 07.09.2026): trưởng nhóm ghi nhận người
...
def competition_rank(items, key):
    """Xếp hạng kiểu thi đấu (Q77): bằng khoá thì đồng hạng, người kế tiếp nhảy
...
def _cap_duoi_trong_pham_vi(actor):
    ...
    người ghi nhận, và không phải chính mình (Q75)."""
...
def recipients(actor, users=None):
    """Ai được người này ghi nhận — FR-12.1, Q75. Nhân viên: không ai.
...
def give_recognition(*, receiver, value, message, actor, request=None):
    """Ghi nhận cấp dưới và cộng sao cùng một giao dịch — FR-12.1, FR-12.2, Q75."""
    if not can_recognize(actor):
