...
`GRID_ONLY_TABLES` giữ mặc định của ERP (`van_don` chỉ xem) để các bài phân
quyền kiểm được cả hai chiều: bảng chỉ xem thì 403, đặt `set()` (đúng cấu hình
dịch vụ 8021 khi chạy thật) thì sửa được. Từ ADR-036 `van_don` là bảng vận đơn
duy nhất, mang profile Vận đơn.
"""
import pytest
