...
def test_migration_0011_an_cot_san_pham_dang_co(client, luoi, nguoi_dung):
    """AC-39.8 — Cột `sl_*` mọc trước 22.09 được migration `orders/0011` ẩn với cả công ty;
...
def test_loc_theo_cot_dang_an_bi_bo_qua(client, luoi, nguoi_dung):
    """AC-39.9 — Hệ quả đã biết của ẩn cột: lưới không đọc bộ lọc của cột đang ẩn, nên
    đường dẫn cũ lọc theo `sl_*` trả về mọi dòng thay vì báo lỗi; bật lại cột thì lọc chạy như xưa"""
    bang, products, rows = luoi
    ...
    def so_dong(qs=""):
        kq = client.get(f"/bang-tinh/{bang.code}/du-lieu/" + qs)
        assert kq.status_code == 200
        return len(kq.json()["rows"])

    tong = so_dong()
    assert ColumnDef.objects.get(table=bang, code=ma).is_hidden is True
    assert so_dong(f"?f_{ma}__trong=1") == tong, "cột ẩn: bộ lọc bị bỏ qua, không lọc gì"

    table_service.set_columns_hidden(bang, [ma], False, actor=nguoi_dung["admin"])
    assert so_dong(f"?f_{ma}__trong=1") == 1, "bật lại cột thì bộ lọc chạy lại"
