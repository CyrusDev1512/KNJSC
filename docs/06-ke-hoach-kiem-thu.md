...
| | Số |
|---|---|
| Tiêu chí nghiệm thu trong `docs/04` | **249** — 236 tự động, 13 thủ công |
| Tiêu chí tự động đã có bài kiểm | **213 trên 236** |
...
| 8 | **Đầu-cuối trình duyệt** | Lưới: `scripts/kiem-thu-master-*.cjs` (Node + Chrome thật); còn lại `tests/e2e/` — Playwright, dấu `trinh_duyet` | Nhập → xuất → nhập lại qua giao diện; bàn phím và hộp lọc trên Bảng tính; kéo chọn vùng, dán TSV, kéo điền, hoàn tác, ô địa chỉ, chuột phải xoá hàng rồi hoàn tác (ADR-011); trang chủ KN CRM bấm tháng → lưới lọc tháng → ← (ADR-012); cột cố định khi cuộn; 390px không tràn ngang, có ảnh chụp. **Một hành trình xuyên màn hình** (`tests/e2e/test_hanh_trinh_nhan_vien.py`, 19.09): Vận đơn đăng nhập → lưới → gõ ô → tải lại kiểm dữ liệu đã xuống DB → đổi hộp lọc cột → đổi vai Sale → lên đơn khách cũ → lời nhắc khách; không thêm AC mới, xác nhận lại AC-11.43, AC-11.42, AC-6.10 ở chỗ nối giữa các màn hình | Đổi phím Esc thành không làm gì trong `bang-tinh.js` → đỏ; trả `hx-target` của mảnh lọc về `#hop-loc`, hoặc vô hiệu listener tự điền tên ở `order-entry.js` → hành trình đỏ (đã thử 19.09) |
