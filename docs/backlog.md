...
| 03.09.2026 | Máy Windows của người dùng mở `localhost:8021` bị từ chối: Docker Desktop chưa chạy và mã còn ở Giai đoạn 2. Sửa `scripts/cap-nhat-local.bat` và `.sh` thành một lệnh tự lo trọn: tự mở Docker Desktop và chờ, `git pull --ff-only` có kiểm lỗi, dựng lại, nạp dữ liệu mẫu, `compose ps`, mở trình duyệt. Ghi vào `docs/05` B6 |
