# Kiểm chứng — Form Nộp báo cáo ngày (ADR-043), 24.09.2026

Nhánh `claude/form-nhap-bao-cao` tách từ `main` (ADR-042 đã gộp qua PR #36), PR nháp riêng. Máy ảo Claude Code trên web: Postgres 16
...
của AC-43.2 viết cho ô chọn trong khi bảng của bài kiểm có Sản phẩm kiểu chữ; hai bộ đếm docs/06 cũ) →
sửa → `test_form_nhap_bao_cao.py` + `test_truy_vet.py` xanh. Bộ đầy đủ `-m "not cham"` từ `app/` sau mọi sửa:
**2.581 đạt, 0 đỏ** trên nền cũ (nhánh ADR-042); sau khi đưa lên `main` (đã gộp 11 PR, ADR-043 đánh số lại):
**2.682 đạt, 9 bỏ qua (bài trình duyệt), 54 bỏ chọn (`cham`), 0 đỏ** trong 5 phút 21 giây.

Bài mới `reports/tests/test_form_nhap_bao_cao.py`: AC-43.1 (team hai chiều: Leader team được chọn thấy và sửa,
Leader team khác 403/404; team bộ phận khác, id lạ bị từ chối; để trống về hồ sơ; bộ phận không team → không ô),
AC-43.2 (MKT bốn trường, Sale ba trường qua `call_command("configure_erp_reports")`; trường đã có bị ép; thiếu
CPQC → "Chưa điền các trường bắt buộc: CPQC", không tạo dòng; "0" hợp lệ; `required` đúng ô), AC-43.3 (gỡ Hóa
đơn, chạy lại không tạo lại, cột/ánh xạ/chỉ tiêu giữ, `hoa_don` gửi thẳng bị bỏ), AC-43.4 (template, CSS,
...
- Màn Sửa báo cáo (`bao_cao_sua`) cùng lưới `bm-ngang` chỉ qua bài AC-43.4 (tệp), chưa chụp.
