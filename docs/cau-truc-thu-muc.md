...
│   │   └── tests/
│   │
│   ├── documents/                         Nội bộ — tài liệu chia mục (ADR-015)
...
| `documents` | Mục tài liệu, tài liệu (tệp ở `storage/tai-lieu/`) — nhóm Nội bộ, ADR-015 | `core`, `org` |
| `taskboard` | Việc | `core`, `org` |
| `culture` | Ghi nhận, sao; bảng xếp hạng đọc đơn hàng (ngoại lệ phạm vi có chủ ý, Q66) | `core`, `org`, `orders` |
