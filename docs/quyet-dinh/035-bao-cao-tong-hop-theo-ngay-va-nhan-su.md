...
5. **Bảng gọn theo token `sheet-body` 13px** của DESIGN.md: chữ 13px, ô đệm 5×8 px, khung
   ...
   > cộng theo ngày) và cột **STT** đếm lại từ 1 trong ngày — xem AC-22.15. Cột chỉ số quan trọng có
   > nền riêng và ô tỉ lệ đổi màu theo chiều tốt so với dòng Tổng (`reports/constants.FOCUS_METRICS`,
   > `METRIC_DIRECTION`, `THRESHOLD_BAND`) — xem AC-22.16. Bỏ `StringAgg` gộp tên và
