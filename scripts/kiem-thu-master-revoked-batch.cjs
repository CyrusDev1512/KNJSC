...
module.exports=async({admin,leader,staff,context,base,delivery})=>{
  const rows=await admin.evaluate(async()=>Promise.all([0,1].map(async n=>{
    const data=await fetch('/bang-tinh/van_don/du-lieu/?f_ma_don=MASTER-0000'+n).then(r=>r.json());return data.rows[0];
  })));
  ...
  await staff.goto(base+'/bang-tinh/van_don/');await staff.bringToFront();
  ...
  const before=await admin.evaluate(async()=>fetch('/bang-tinh/van_don/du-lieu/?f_ma_don=MASTER-00001').then(r=>r.json()));
