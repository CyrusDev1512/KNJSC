...
try{
 ...
 // Đúng thứ tự base_bang_tinh + master_grid; thiếu Solarpunk sẽ bỏ sót lỗi kế thừa theme.
 const css=['tokens.css','main.css','crm-frame.css','grid-formats.css','master-grid.css','waybill.css','solarpunk.css'].map(name=>fs.readFileSync(path.join(root,'app/static/css',name),'utf8')).join('\n');
 for(const width of [1440,390])for(const theme of ['dark','light']){
  await page.setViewportSize({width,height:900});
  const columns=[['so_dien_thoai','Số điện thoại','0900000001'],['ma_don','Mã đơn','DH-TEST-0001'],['ten_khach','Tên khách','Khách kiểm thử']];
  const cells=columns.map(([code,label,value])=>`<div class="mg-heading mg-pinned" data-code="${code}" style="position:relative;width:200px"><button>${label}</button></div><div class="mg-cell mg-pinned" data-code="${code}" style="position:relative;width:200px">${value}</div>`).join('');
  await page.setContent(`<html data-theme="${theme}"><head><style>${css}</style></head><body class="bt-than sp-grid"><div class="mg-root mg-waybill-master"><div class="mg-viewport" style="height:400px">${cells}<div class="mg-cell" style="position:relative;width:200px">Ô thường</div></div></div></body></html>`);
  ...
  results.push({theme,width,colors,ratios});await page.screenshot({path:path.join(out,theme+'-'+width+'.png')});
 }
