...
const root=path.resolve(__dirname,'..'),base='http://127.0.0.1:8035',out=path.join(root,'.agents/design-state/review/master-nine');
...
(async()=>{
  ...
  try{
    ...
    // Luồng thật nhiều vai trò: Admin tạo, Leader giao, Staff lưu, Sale xem lịch sử.
    await page.goto(base+'/van-don/len-don/');await page.locator('[name=customer_name]').fill('Khách E2E chín hạng mục');await page.locator('[name=phone]').fill('0912345999');
    ...
    await sale.ctx.close();await staff.ctx.close();await leader.ctx.close();evidence.checks.push('E2E Admin → cuối bảng → Leader phân công → Staff autosave → Sale xem lịch sử; thu quyền gỡ cả nháp');
